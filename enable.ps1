###################################################
# HelloID-Conn-Prov-Target-NokiLock-Employees-Enable
# PowerShell V2
###################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Enabling NokiLock-Employees account with accountReference: [$($actionContext.References.Account)]"
                ...
                Write-Information "[DryRun] Enable NokiLock-Employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    $outputContext.success = $false
    $ex = $PSItem
    $auditMessage = "Could not enable NokiLock-Employees account. Error: $($_.Exception.Message)"
