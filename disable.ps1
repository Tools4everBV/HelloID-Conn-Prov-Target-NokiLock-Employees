####################################################
# HelloID-Conn-Prov-Target-NokiLock-Employees-Disable
# PowerShell V2
####################################################
...
try {
    ...
    Write-Information 'Verifying if a NokiLock-Employees account exists'
    $splatParams = @{
    ...
    if ($response.LockerApi.function.return.user) {
        ...
        $CardAssignments = $CardAssignments | Where-Object blocked -ne $true
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (($CardAssignments | measure-object).count -gt 0) {
                foreach ($CardAssignment in $CardAssignments) {
                    ...
                    $splatUpdateParams = @{
                        Uri        = "$($actionContext.Configuration.BaseUrl)/rpc/xml/blockCard"
                        Method     = 'POST'
                        Headers    = @{'Content-Type' = 'application/x-www-form-urlencoded' }
                        Body       = $body
                        WebSession = $sessionContext.Session
                    ...
                    if (-not($actionContext.DryRun -eq $true)) {
                        Write-Information "Disabling NokiLock-CardAssignment for cardID [$($CardAssignment.cardID)]  and lockerBlock [$($CardAssignment.lockerblockID)]"
                        ...
                        Write-Information "[DryRun]: Disable NokiLock-cardAssignment for cardID [$($CardAssignment.cardID)] and lockerBlock [$($CardAssignment.lockerblockID)], will be executed during enforcement"
                    }
                }
            }
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling NokiLock-Employees account with accountReference: [$($actionContext.References.Account)]"
                $response = Invoke-RestMethod @splatUpdateParams -Verbose:$false
                if ($response.LockerAPI.error.message) {
                    throw [System.Exception]::new("LockerAPI Failed [$($splatUpdateParams.uri)]. error: $($response.LockerAPI.error.message)")
                }
                ...
                Write-Information "[DryRun] Disable NokiLock-Employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
        ...
        'NotFound' {
            Write-Information "NokiLock-Employees account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted"
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "NokiLock-Employees account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted"
                    IsError = $false
                })
            break
        }
    ...
    $ex = $PSItem
    $auditMessage = "Could not disable NokiLock-Employees account. Error: $($_.Exception.Message)"
