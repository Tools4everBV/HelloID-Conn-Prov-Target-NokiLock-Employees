###################################################
# HelloID-Conn-Prov-Target-NokiLock-Employees-Update
# PowerShell V2
###################################################
...
try {
    ...
    switch ($action) {
        'UpdateAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Updating NokiLock-Employees account with accountReference: [$($actionContext.References.Account)]"
        ...
        'NoChanges' {
            Write-Information "No changes to NokiLock-Employees account with accountReference: [$($actionContext.References.Account)]"
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
    ...
    $outputContext.success = $false
    $ex = $PSItem
    $auditMessage = "Could not update NokiLock-Employees account. Error: $($_.Exception.Message)"
