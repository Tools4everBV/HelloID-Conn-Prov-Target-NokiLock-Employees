###################################################
# HelloID-Conn-Prov-Target-NokiLock-Employees-Delete
# PowerShell V2
###################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting NokiLock-Employees account with accountReference: [$($actionContext.References.Account)]"
                ...
                Write-Information "[DryRun] Delete NokiLock-Employees account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    $ex = $PSItem
    $auditMessage = "Could not delete NokiLock-Employees account. Error: $($_.Exception.Message)"
